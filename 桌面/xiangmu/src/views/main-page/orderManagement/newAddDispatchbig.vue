  ...
  <div class="audit">
    <div class="table_main top20">
      <div class="head_title">
        <h3 class="newAddTitle">{{title}}预派单(大宗)</h3>
      ...
      <el-form :model="productInfo" :rules="rules" ref="productInfo" class="demo-dynamic">
        ...
        <div class="projectBox">
          ...
          <ul class="flex">
            ...
            <!-- 业务类型 -->
            <li>
              <el-form-item prop="projectBusinessTypeName">
                <!--
            ...
            <li>
              <el-form-item class="width100" style="float:left;" prop="orderFreightTotal">
                <el-input
                  ...
                  placeholder="派车量"
                ></el-input>
                <span v-else  >{{this.productInfo.orderFreightTotal}}</span>
              </el-form-item>

              <el-form-item class="width80" style="position:absolute;right:40px;" prop="orderFreightUnit">
                <!--
                <el-select
                  class="width80 nobor"
        ...
        <div class="projectBox">
          <ul class="flex">
            ...
            <li>物料体积</li>
            <li>总件数</li>
            <li class="wuInfoChoose" v-if="showBtn"></li>
            <li class="wuInfoChoose" v-if="showBtn">
              <el-form-item>
          ...
          <ul class="flex" v-for="(item,index) in productInfo.orderGoodsList" :key="index">
            ...
            <li class="noborder">
              <el-form-item>
                <el-input class="width100" style="float:left" v-model="item.goodsSettlementTotal" placeholder="计费量"></el-input>
        ...
        <div v-if="isBigZong" class="projectBox">
          <ul class="flex">
            ...
            <li>涨吨扣款比例</li>
            <li>涨吨扣款标准</li>
            <li  v-if="productInfo.projectSettlementWay == 2 && showBtn"></li>
          </ul>
          <!-- ------------------------------------------ -->
          <ul class="flex">
            <li>
            ...
            <li  v-if="productInfo.projectSettlementWay == 2 && showBtn">
    ...
    <!-- 单价信息 -->
    <el-dialog title="选择价格表" width="640px" :visible.sync="fourthDialog">
      <el-container>
        <el-header>
          <el-input placeholder="请输入内容" v-model="searchInput" class="input-with-select width300">
            <el-button slot="append" icon="el-icon-search"></el-button>
            ...
            >
              <div v-if="isBigZong">
                ...
                <div class="flex">
                  ...
                  <div class="item">
                    <p class="projectTitle">
                      <span v-if="productInfo.projectSettlementWay == 1">订单单价</span>
                      <span v-if="productInfo.projectSettlementWay == 2">运单单价</span>
    ...
    return {
     ...
     // 时间
      ...
      productInfo: {
        ...
        orderUnloadLon: "", // 卸货经度  核实
        orderUnloadLat: "", // 卸货纬度  核实
...
2、无差额需要选择订单计算公式、
    ...
    chooseItemBtn() {
      if (this.classIndex >= 0) {
        ...
        this.productInfo.projectSettlementClass = this.currentItem.projectSettlementClass; // 计费分类[也就是对应的结算业务]（1大宗、2快消；选择大宗时，有亏涨吨
          this.productInfo.projectSettlementClass = this.currentItem.projectSettlementClassName;
          this.isBigZong = this.productInfo.projectSettlementClass == 1 ? true : false;
    ...
    chooseItemBtn4(classIndex) {
      if (classIndex >= 0) {
        ...
        this.productInfo.priceType =    this.currentItem.priceType||1;
    ...
    newProjectChoose(index) {
      ...
      if (index == 5) {
        ...
        url = this.$api.Basics.priceList +
            "?page=1&rows=20" 
            + "&priceType=2&project.projectName=" + this.productInfo.projectName;
      }
    ...
    getGoodsItem(index) {
      ...
      this.wuliao = true;
      let url =
      this.$api.Basics.materiel + "?page=1&rows=20" 
      + "&project.projectName=" + this.productInfo.projectName;
      this.getOrderInfo(url);
    },
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let newProductInfo = {
            // 项目信息字段
            truckingProjectId: this.productInfo.orderProjectId, // 项目ID
            orderId: this.productInfo.orderId, // 订单ID
            orderCode: this.productInfo.orderCode, // 订单编号

            truckingOrderPriceId : this.productInfo.orderPriceId,
            truckingFreightTotal: parseFloat(this.productInfo.orderFreightTotal), // 派车量 计费总量
            truckingFreightUnit: this.productInfo.orderFreightUnit, // 单位 计费总量单位

            // orderProject: {
            //   projectName: this.productInfo.projectName, //项目名称
            //   projectBusinessType: this.productInfo.projectBusinessType == "干线普货运输" ? 1002996 : 0, //业务类型
            //   projectCustomer: this.productInfo.projectCustomer, // 客户名称
            //   projectSettlementWay:
            //     this.productInfo.projectSettlementWay == '无差额' ? 1 : this.productInfo.projectSettlementWay == '有差额代管' ? 2 : 3, // 结算类型
            //   projectSettlementClass: this.productInfo.projectSettlementClass == '大宗' ? 1 : 2, //计费分类
            //   projectCalculateWay: this.productInfo.projectCalculateWay == '系统计算' ? 1 : 2, // 运费计算模式
             ...
             **/
            truckingPrice: { //zo
              priceType: this.productInfo.priceType||1,
            ...
            /***
              其它信息
            **/
            truckingLoadTime: this.productInfo.orderLoadTime, // 要求提货时间
            truckingUnloadTime: this.productInfo.orderUnloadTime, // 要求卸货时间
            // truckingUnloadSettlement:this.productInfo.truckingUnloadSettlement, // 是否以签收量计算运费
            truckingManagementAmt: this.productInfo.truckingManagementAmt, // 管理费
            truckingOtherAmtDesc: this.productInfo.truckingOtherAmtDesc, //
            truckingOtherAmt: this.productInfo.truckingOtherAmt, //
            truckingAdvanceCard: this.productInfo.truckingAdvanceCard, //
            truckingAdvanceAmt: this.productInfo.truckingAdvanceAmt,
    ...
    // 创建订单立即创建
    getData(data) {
      // console.log(JSON.stringify(data), '新建预派车单')
      ...
      this.$http
        .post(this.$api.Order.addDispatch, data, true)
        .then(res => {
          if(res.data.code > 0) {
            this.$message({
              showClose: true,
  ...
  mounted() {
    if(this.$route.query.orderId) {
      ...
      this.$http.post(this.$api.Order.getDispatchOfOrderId, param, true)
        .then(res => {
          if(res.data.code == 1) {
            ...
            this.productInfo.projectCalculateWay = orderDetail.orderProject.projectCalculateWay;
              this.productInfo.projectCalculateWayName = orderDetail.orderProject.projectCalculateWayName// 运费计算模式
             ...
             * 发货人信息
             * */
            this.productInfo.orderLoadAddressId = orderDetail.orderLoadAddress.orderAddressId;
            this.productInfo.orderLoadAreaId = orderDetail.orderLoadAddress.addressAreaId; // 发货地址区县ID
            this.productInfo.orderLoadArea = orderDetail.orderLoadAddress.addressAreaId; // 发货地址区县  核实
            this.productInfo.orderLoadAddress = orderDetail.orderLoadAddress.addressDetail; // 发货地址  核实
            this.productInfo.orderLoadLon = orderDetail.orderLoadAddress.addressLon; // 发货经度  核实
            this.productInfo.orderLoadLat = orderDetail.orderLoadAddress.addressLat; // 发货纬度  核实
            this.productInfo.orderLoadCompany = orderDetail.orderLoadAddress.addressCompany; //发货单位  核实
            this.productInfo.orderLoadContacter = orderDetail.orderLoadAddress.addressContacter; //发货联系人  核实
            this.productInfo.orderLoadContactMobile = orderDetail.orderLoadAddress.addressContactMobile; // 发货人电话  核实
            this.sendCityName = this.getProvinceName(orderDetail.orderLoadAddress.addressAreaId + '');
            /**
             * 收货人信息
             * */
            this.productInfo.orderUnLoadAddressId = orderDetail.orderUnLoadAddress.orderAddressId;
            this.productInfo.orderUnloadAreaId = orderDetail.orderUnLoadAddress.addressAreaId; // 卸货区县ID
            this.productInfo.orderUnloadArea = orderDetail.orderUnLoadAddress.addressAreaId; // 卸货区县 核实
            this.productInfo.orderUnloadDetail = orderDetail.orderUnLoadAddress.addressDetail; // 卸货详细地址 核实
            this.productInfo.orderUnloadLon = orderDetail.orderUnLoadAddress.addressLon; // 卸货经度  核实
            this.productInfo.orderUnloadLat = orderDetail.orderUnLoadAddress.addressLat; // 卸货纬度  核实
            this.productInfo.orderUnloadCompany = orderDetail.orderUnLoadAddress.addressCompany; // 卸货公司  核实
            this.productInfo.orderUnloadContacter = orderDetail.orderUnLoadAddress.addressContacter; // 卸货联系人  核实
            this.productInfo.orderUnloadContactMobile = orderDetail.orderUnLoadAddress.addressContactMobile; // 卸货联系人电话  核实
            // this.productInfo.orderUnloadContactIdentity = orderDetail.orderUnLoadAddress.addressContactIdentity; //收货人信用代码或个人证件号  核实
            this.unLoadCityName = this.getProvinceName(orderDetail.orderUnLoadAddress.addressAreaId + '');
            ...
            if(this.productInfo.projectSettlementWay == 1){
                this.productInfo.priceName = orderDetail.orderPrice.priceName;// 价格/合同 名称
                this.productInfo.priceSettlementUnit = orderDetail.orderPrice.priceSettlementUnit;//计费单位
                this.productInfo.priceSettlementUnitprice = orderDetail.orderPrice.priceSettlementUnitprice;//订单单价
                this.productInfo.priceDeficitWay = orderDetail.orderPrice.priceDeficitWay;//亏吨计量方式
                this.productInfo.priceDeficitValue = orderDetail.orderPrice.priceDeficitValue;// 亏吨允许比列
                this.productInfo.priceDeficitUnitprice = orderDetail.orderPrice.priceDeficitUnitprice;// 亏吨扣款标准
                this.productInfo.priceRiseWay = orderDetail.orderPrice.priceRiseWay;// 涨吨计量方式
                this.productInfo.priceRiseValue = orderDetail.orderPrice.priceRiseValue;//涨吨允许比例
                this.productInfo.priceRiseUnitprice = orderDetail.orderPrice.priceRiseUnitprice;// 涨吨扣款标准
            }

            /**
             * 非大宗合同
             * **/
            // this.productInfo.priceDamageDeduct = orderDetail.orderPrice.priceDamageDeduct;// 货损扣款
            // this.productInfo.priceOverdueDeduct = orderDetail.orderPrice.priceOverdueDeduct;// 晚到扣款
            // this.productInfo.priceOtherDeduct = orderDetail.orderPrice.priceOtherDeduct;// 其他扣款
